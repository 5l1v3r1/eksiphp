...
class eksi {
    ...
    private function login(&$ch,$email, $pass, $data){
    ...
    function getReqEntry ($kuki, $req1) {
        $ch2 = curl_init();
        curl_setopt($ch2, CURLOPT_URL, 'https://eksisozluk.com/php--41850');
        curl_setopt($ch2, CURLOPT_HEADER, true);
        curl_setopt($ch2, CURLOPT_HTTPHEADER, array("Cookie: __gfp_64b=ADJ3JN5eiCcFFHzS0qms7uV6WPHWaO8Ibg3GwZ0Uxu3.57; _ga=GA1.2.1733273535.1576951415; _gid=GA1.2.1161511079.1576951415; ASP.NET_SessionId=led1i5kytz4pvl25mq2sqxtn; iq=1b32ef204ef44561991aff85ff309c89; __RequestVerificationToken=$req1; sticky_id=82a3c316ed3e3d8a1e5076a1756ad1e5; a=$kuki"));
        curl_setopt($ch2, CURLOPT_FOLLOWLOCATION, true);
        curl_setopt($ch2, CURLOPT_RETURNTRANSFER, true);
        ...
        preg_match('/name="__RequestVerificationToken" type="hidden" value="(.*?)" /is', $GirisSonuc, $regs);
    ...
    function girisYap($email, $pass) {
        ...
        $data = $this->login($ch,$email,$pass, $data);
    ...
    function sendEntry ($baslik, $entry, $req1) {
        $kuki = $this->cookie;
        $Id = explode("--", $this->getBaslikID($baslik))[1];
        $getreq = $this->getReqEntry($kuki, $req1);
        ...
        curl_setopt($ch2, CURLOPT_POSTFIELDS, "__RequestVerificationToken=$getreq&Title=$baslik&Id=$Id&ReturnUrl=&InputStartTime=22.12.2019+14%3A10%3A50&Content=$entry");     
        curl_setopt($ch2, CURLOPT_USERAGENT,'Mozilla/5.0 (Windows; U; Windows NT 5.1; en-US; rv:1.8.1.13) Gecko/20080311 Firefox/2.0.0.13');
        $GirisSonuc = curl_exec($ch2);

        $Yon = preg_match_all('/^Location:(.*)$/mi',$GirisSonuc,$matches);
        $url = $matches[1][0];
        $HTTPCode = curl_getinfo($ch2, CURLINFO_HTTP_CODE);

        if (strstr($url, "/entry/")) {
            return str_replace("/entry/", "", $url);
        } else {
    ...
    function getEntry ($num) {
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, 'https://eksisozluk.com/entry/'.$num.'');
        curl_setopt($ch, CURLOPT_HEADER, true);
        curl_setopt($ch, CURLOPT_FOLLOWLOCATION, true);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        $EntrySonuc = curl_exec($ch);
        $httpcode = curl_getinfo($ch, CURLINFO_HTTP_CODE);

        if ($httpcode == 404 || strstr($EntrySonuc, "<title>büyük başarısızlıklar sözkonusu - ekşi sözlük</title>")) {
            echo "<b> Hata Kodu: 7 </b> <br> Böyle bir entry yok.\n Sunucudan dönen hata: $httpcode"; 
        } else {
			$DataFavoriteCount = "data-favorite-count";
			$DataAuthor = "data-author";  // Aaaah PHP sytnaxı
			$DataSeylerSlug = "data-seyler-slug";
			$DataAuthorId = "data-author-id";

			$Entry = str_get_html($EntrySonuc);
			$Yazar = $Entry->find('#topic')[0]->find('ul')[0]->find('li')[0]->$DataAuthor;
			$Fav = $Entry->find('#topic')[0]->find('ul')[0]->find('li')[0]->$DataFavoriteCount;
			$Seyler = $Entry->find('#topic')[0]->find('ul')[0]->find('li')[0]->$DataSeylerSlug;
			$YazarID = $Entry->find('#topic')[0]->find('ul')[0]->find('li')[0]->$DataAuthorId;
			
			$Giri = $Entry->find('#topic')[0]->find('ul')[0]->find('li')[0]->find(".content")[0]->innertext;
			$Tarih = $Entry->find("a[class=entry-date permalink]")[0]->innertext;
		
			$Array = ["id" => $num, "entry" => strip_tags($Giri), "sahibi" => $Yazar, "sahibi_id" => $YazarID, "fav" => $Fav, "seyler" => $Seyler, "zaman" => $Tarih];    
    ...
    function Gundem($Entry) {
        ...
        if (isset($Entry)) {
            if (is_numeric($Entry)) {
                ...
                if (strstr($Giri, "b")) {
                    if (strstr($Giri, ",")) {
                        $Giriz = str_replace(",", "", str_replace("b", "00", $Giri));
                    } else {
                        $Giriz = str_replace("b", "000", $Giri);
                    }
                } else {
                    $Giriz = $Giri; 
                }
                ...
                $Array = ["baslik" => $Baslik, "giri" => $Giriz, "link" => $Link];
                ...
                for ($i = 0; $i <= 55; $i++) {
                    ...
                    $Array[$i] = ["baslik" => $Baslik, "giri" => $Giriz, "link" => $Link];
                }
